...
BEGIN
    ...
    v_ingestion_id := CONCAT('ING_', TO_VARCHAR(CURRENT_TIMESTAMP, 'YYYYMMDDHH24MISS'), '_', UNIFORM(1000, 9999, RANDOM()));
    ...
    BEGIN
        ...
        SELECT 
            seq4() as product_id,
            'Sample Product ' || seq4() as product_name,
            CASE (seq4() % 3) 
        ...
        SELECT 
            ...
            '+1-555-' || LPAD(seq4()::STRING, 4, '0') as contact_number,
            (seq4() % 100) + 1 as product_id,
            CURRENT_TIMESTAMP as load_date,
            CURRENT_TIMESTAMP as update_date,
            p_source_system as source_system
        ...
        SELECT 
            seq4() as warehouse_id,
            'Warehouse Location ' || seq4() as location,
            (seq4() % 10000) + 1000 as capacity,
            CURRENT_TIMESTAMP as load_date,
            CURRENT_TIMESTAMP as update_date,
            p_source_system as source_system
        ...
        SELECT 
            ...
            (seq4() % 100) + 1 as product_id,
            (seq4() % 1000) + 1 as quantity_available,
            (seq4() % 10) + 1 as warehouse_id,
            CURRENT_TIMESTAMP as load_date,
            CURRENT_TIMESTAMP as update_date,
            p_source_system as source_system
    ...
    EXCEPTION
        WHEN OTHER THEN
            ...
            INSERT INTO bz_error_log (
                ...
                v_ingestion_id, 'BATCH_PROCESSING', 'PROCESSING_ERROR', v_error_message, v_current_user
            );
    END;
    ...
    RETURN CONCAT(
        ...
        ', Records Processed: ', v_total_records_processed,
        ', Records Failed: ', v_total_records_failed,
