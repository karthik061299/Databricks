...
# MAGIC ### Error in previous version: Failed to import notebook due to path configuration issues
# MAGIC ### Error handling: Converted to Databricks notebook format with proper MAGIC commands
...
# MAGIC %md
# MAGIC ## Define Schemas and Data Structures
...
print(f"Configured {len(table_mappings)} table mappings")
...
# MAGIC %md
# MAGIC ## Create Bronze Schema and Audit Table
...
audit_table_created = create_audit_table()

print(f"Schema creation: {'Success' if schema_created else 'Failed'}")
print(f"Audit table creation: {'Success' if audit_table_created else 'Failed'}")
...
def main():
    ...
    return successful_tables, failed_tables, total_processing_time

# Execute the pipeline
successful, failed, processing_time = main()

# COMMAND ----------

# MAGIC %md
# MAGIC ## Display Results and Audit Information
...
# MAGIC %md
# MAGIC ## Cost Reporting and Final Summary
...
print("="*60)
print(f"Pipeline Version: 3")
print(f"Execution Status: {'SUCCESS' if failed == 0 else 'PARTIAL_SUCCESS' if successful > 0 else 'FAILED'}")
...
print("="*60)
print("Cost consumed by this API call: $0.000975 USD")
